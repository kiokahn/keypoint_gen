...
void Camera::computeVanishingLine()
{
    ...
    vec3 dir = camera_pose_.basis[2];

    vec3 p = camera_pose_.eye_position + dir * 1000000.0;
    vec2 p1 = projectToImagePlane(p + vec3(0, 0, 0));
    vec2 p2 = projectToImagePlane(p + vec3(1, 0, 0));
    vec2 p3 = projectToImagePlane(p + vec3(0, 1, 0));
    vec2 p4 = projectToImagePlane(p + vec3(1, 1, 0));

    vec2 p5 = projectToImagePlane(p + vec3(0, 0, 1));
    vec2 its;
    int ret = intersect(p1, p2, p3, p4, its);

    if (degree(graphics::angle(dir, vec3(0, 0, -1))) < 2.0) {
        horizon_line_y_ = 0;
        horizon_reversed_ = true;
        return;
    }
