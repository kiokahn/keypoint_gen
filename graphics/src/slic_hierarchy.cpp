...
void slic_hierarchy::build(unsigned int* img, int w, int h, int l0, int l1, int compact)
{
	hierarchy_.clear();
...
void slic_hierarchy::build(unsigned int* img, int w, int h, int l0, int compact)
{
	hierarchy_.clear();
	std::vector<int> levels(1);
	levels[0] = l0;

	std::vector<int> num_labels(1);
	int i;
	hierarchy_.resize(1);
	int sz = w*h;
