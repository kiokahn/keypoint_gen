	...
	const double&				compactness)
{
    ...
    std::vector<double> sigmax(numk, 0);
    std::vector<double> sigmay(numk, 0);
    std::vector<double> sigmaz(numk, 0);

    std::vector< double > initdouble(sz, DBL_MAX);
    std::vector< std::vector<double> > distvec(m_depth, initdouble);
	...
	for( int itr = 0; itr < 5; itr++ )
	{
		...
		for( int n = 0; n < numk; n++ )
		{
                        ...
                        x1 = std::max(0.0,			kseedsx[n]-offset);
                        x2 = std::min((double)m_width,	kseedsx[n]+offset);
                        z1 = std::max(0.0,			kseedsz[n]-offset);
                        z2 = std::min((double)m_depth,	kseedsz[n]+offset);
...
#else
    std::string nameandextension = filename;
        size_t pos = filename.find_last_of("/");
        if(pos != std::string::npos)//if a slash is found, then take the filename with extension
        {
            nameandextension = filename.substr(pos+1);
        }
        std::string newname = nameandextension.replace(nameandextension.rfind(".")+1, 3, "dat");//find the position of the dot and replace the 3 characters following it.
