...
void gl_stat::save_stat()
{
	...
	glGetIntegerv(GL_READ_FRAMEBUFFER_BINDING, &read_frame_buffer_);
	glGetIntegerv(GL_DRAW_FRAMEBUFFER_BINDING, &draw_frame_buffer_);
...
void gl_stat::restore_stat()
{
	...
	glBindFramebuffer(GL_DRAW_FRAMEBUFFER, draw_frame_buffer_);
	glBindFramebuffer(GL_READ_FRAMEBUFFER, read_frame_buffer_);
	glDrawBuffer(draw_buffer_);
