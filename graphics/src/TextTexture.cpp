...
namespace graphics
{
	...
	void TextTexture::SetText(const std::string& str, const graphics::vec4& fg_color, const graphics::vec4& bg_color, int font_size, int label_width, int align)
	{
		...
		for (QTextBlock it = doc.begin(); it != doc.end(); it = it.next())
		{
			...
			while (line.isValid()) {
				...
				height += line.height();
				total_width = max(total_width, (int)line.width());
		...
		QColor black(int(bg_color[0]*255.0), int(bg_color[1] * 255.0), int(bg_color[2] * 255.0), int(bg_color[3] * 255.0));
		img.fill(black);

		QPainter painter;
		painter.begin(&img);

		painter.setPen(QColor(int(fg_color[0] * 255.0), int(fg_color[1] * 255.0), int(fg_color[2] * 255.0), int(fg_color[3] * 255.0)));
	...
	void TextTexture::SetText(const std::wstring& str, const graphics::vec4& fg_color, const graphics::vec4& bg_color, int font_size, int label_width, int align)
	{
		...
		Qt::Alignment data_halign = (align == 0) ? Qt::AlignLeft : (align == 1) ? Qt::AlignCenter : Qt::AlignRight;
		QString data_message = QString::fromStdWString(str);
		...
		QImage img(total_width, height, QImage::Format_ARGB32_Premultiplied);
		QColor black(bg_color[0] * 255, bg_color[1] * 255, bg_color[2] * 255, bg_color[3] * 255);
		img.fill(black);

		QPainter painter;
		painter.begin(&img);

		painter.setPen(QColor(fg_color[0] * 255, fg_color[1] * 255, fg_color[2] * 255, fg_color[3] * 255));
		painter.setFont(data_ft);
