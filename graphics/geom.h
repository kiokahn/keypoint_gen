...
struct box2 {
	...
	box2(const vec4& val)
	{
		minimum = vec2(val[0], val[1]);
		maximum = minimum + vec2(val[2], val[3]);
	}
	...
	void get(vec4& val)const {
		val[0] = minimum[0];
		val[1] = minimum[1];
		val[2] = maximum[0] - minimum[0];
		val[3] = maximum[1] - minimum[1];
	}
    ...
    bool is_empty() const { return maximum[0] <= minimum[0] || maximum[1] <= minimum[1]; }
	...
	int corner_count() const { return 4; }
	vec2 corner(int i) const { 
		real sizex = maximum[0] - minimum[0];
		real sizey = maximum[1] - minimum[1];
		if (i == 0) return minimum; 
		if (i == 1) return minimum + vec2(sizex, 0);
		if (i == 2) return minimum + vec2(sizex, sizey);
		if (i == 3) return minimum + vec2(0, sizey);
		return vec2(0);
	}
};
...
struct box3 {
	...
	vec3 eval(const vec3& param) const {
		return maximum*param + minimum*(vec3(1) - param);
	}
	...
	// apx bounding sphere radius
	real  get_radius() const {
		real x = (maximum[0] - minimum[0])*0.5;
		real y = (maximum[1] - minimum[1])*0.5;
		real z = (maximum[2] - minimum[2])*0.5;
		x = max(max(x, y), z);
		return x;
	}
	real get_size() const { return (maximum[0] - minimum[0])*(maximum[1] - minimum[1])*(maximum[2] - minimum[2]); }
    void  make_empty();

    bool is_empty() const { return maximum[0] <= minimum[0] || maximum[1] <= minimum[1] || maximum[2] <= minimum[2]; }
...
plane ransac_plane(const std::vector<vec3>& points, real max_error = 1.0e-8, real outlier_pro = 0.001);
plane ransac_plane_as_number(const std::vector<vec3>& points, real max_error = 1.0e-8, real outlier_pro = 0.001);
line2 ransac_line(const std::vector<vec2>& points, real max_error = 2.0, real outlier_pro = 0.001);
line  ransac_line(const std::vector<vec3>& points, real max_error = 1.0e-8, real outlier_pro = 0.001);
vec3  ransac_circle(const std::vector<vec2>& points, real max_error = 1.0e-8, real outlier_pro = 0.001);
